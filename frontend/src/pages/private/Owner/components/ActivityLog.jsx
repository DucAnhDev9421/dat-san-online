    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 16, marginBottom: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 6px 20px rgba(0,0,0,.06)" }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 800, color: "#059669" }}>{todayLogs}</div>
        </div>
        <div style={{ background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 6px 20px rgba(0,0,0,.06)" }}>
            ...
            <tbody>
              {filteredActivityLogs.map((log) => (
                <tr key={log.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  ...
                  <td style={{ padding: 12 }}>
                    <span style={{
                      background: log.user === "Nguyễn Văn Chủ" ? "#e6f3ff" : 
                                 log.user.includes("Nguyễn Thị") || log.user.includes("Trần Văn") ? "#fef3c7" : "#e6f9f0",
                      color: log.user === "Nguyễn Văn Chủ" ? "#1d4ed8" : 
                            log.user.includes("Nguyễn Thị") || log.user.includes("Trần Văn") ? "#d97706" : "#059669",
                      padding: "4px 8px",
                      borderRadius: 999,
                      fontSize: 12,
                      fontWeight: 600,
                  ...
                  <td style={{ padding: 12, fontWeight: 600, color: "#3b82f6" }}>{log.target}</td>
                  ...
                  <td style={{ padding: 12, whiteSpace: "nowrap" }}>
                    <ActionButton
                      bg="#06b6d4"
                      Icon={Eye}
                      onClick={() => alert("Xem chi tiết " + log.id)}
                      title="Xem chi tiết"
                    />
                    <ActionButton
                      bg="#ef4444"
                      Icon={Trash2}
                      onClick={() => alert("Xóa " + log.id)}
                      title="Xóa"
                    />
                  </td>
                </tr>
